            ...
            if (prevCh !== "#" && ch === "{") {
                ...
                if (t[0] === "$") {
                    ...
                    if (t.includes("@include")) {
                        ...
                        arg = arg.substring(0, arg.length - 1);
                        arg = arg.split(",");
                        tProp.push({type: "include", value: c[1].split("(")[0], args: arg});
                    } else {
    ...
    static compile(str) {
        ...
        let compile = (node) => {
            if (node.type === "mixin") {
                ...
                let tokens = math.lex(node.condition).map((item) => {
                    if (item[0] === "$") {
                ...
                //interpalation
                cs = interpalation.lex(cs).map((item) => {
                    if (item[0] === "$") {
                        item = getVariable(item);
                    }
                    return item
                ...
                for (let i = 0; i <= node.props.length - 1; i++) {
                    if (node.props[i].type === "include") {
                        ...
                        //interpalation
                        c[1] = interpalation.lex(c[1]).map((item) => {
                            if (item[0] === "$") {
                                item = getVariable(item);
                            }
                            return item
